...
- **Push is non-fatal**: `write_file` always commits locally, then attempts to push; a push failure (offline, rejected ref, no `origin`) prints a warning and returns `Ok`, so a note is never lost.
